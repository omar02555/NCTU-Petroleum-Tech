(function ($) {
  ...
  var WidgetDefaultHandler = function ($scope) {
    ...
    if ($scope.find(".nisoz-masonary").length) {
      $scope.find(".nisoz-masonary").imagesLoaded(function () {
        $scope.find(".nisoz-masonary").isotope({
          layoutMode: "masonry"
        });
      });
    }
    ...
    // Accrodion
    if ($scope.find(".nisoz-accrodion").length) {
    ...
    let thmOwlCarousels = $scope.find(".nisoz-owl__carousel");
  ...
  $(window).on("elementor/frontend/init", function () {
    ...
    elementorFrontend.hooks.addAction(
      "frontend/element_ready/nisoz-faq.default",
      WidgetFaqHandler
    );
  ...
  // login
  $("#nisoz-login").submit(function (event) {
    event.preventDefault();

    var login = "action=signup_paragon&param=login&" + $(this).serialize();
    $.ajax({
      type: "POST",
      url: nisoz_login_object.ajaxurl,
      data: login,
      beforeSend: function () {
        // setting a timeout
      ...
      success: function (data) {
        ...
        if (data.status == 2) {
          ...
          window.location.href = nisoz_login_object.login_redirect_url;
        } else if (data.status == 1) {
          $(".login-result").html(data.message).addClass("alert alert-warning");
        } else {
          $(".login-result")
            .html(nisoz_login_object.message)
  ...
  // register
  $("#nisoz-registration").submit(function (event) {
    event.preventDefault();
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.status == 2) {
          ...
          window.location.href = nisoz_login_object.registration_redirect_url;
        } else {
          $(".registration-result")
            .html(data.message)
            .addClass("alert alert-warning");
